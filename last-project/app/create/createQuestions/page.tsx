  ...
  const handleAddQuestion = async () => {
    ...
    try {
      ...
      Swal.fire({
        ...
        text: "Failed to create question: " + (err instanceof Error ? err.message : "Unknown error"),
        confirmButtonColor: "#3b82f6",
      });
    } finally {
      setSubmitting(false);
  ...
  const handleGenerateBulk = async () => {
    ...
    try {
      ...
      Swal.fire({
        ...
        text: "Failed to generate questions: " + (err instanceof Error ? err.message : "Unknown error"),
        confirmButtonColor: "#3b82f6",
      });
    } finally {
      setLoading(false);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Swal.fire({
        ...
        text: "Failed to submit questions: " + (err instanceof Error ? err.message : "Unknown error"),
        confirmButtonColor: "#3b82f6",
      });
    } finally {
      setSubmitting(false);
      ...
      <main className="flex-1 ml-64 p-8">
        ...
        <header className="flex justify-between items-center mb-10">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-800">Bulk Management</h1>
          </div>
        ...
        <div className="max-w-5xl">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 mb-10">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-600 mb-2">Question Content</label>
              <textarea 
                disabled={submitting}
            ...
            >
              {submitting ? <Loader2 className="animate-spin w-4 h-4" /> : <PlusCircle className="w-4 h-4" />}
          ...
          {mode === "bulk" && (
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 mb-10">
            <h3 className="font-bold text-slate-800 mb-4">Generate Questions in Bulk</h3>
            <p className="text-sm text-slate-600 mb-4">AI will generate questions based on your criteria</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div>
                <label className="block text-sm font-medium text-slate-600 mb-2">Category</label>
                <select 
            ...
            <button 
              ...
              disabled={loading}
              className="bg-blue-600 text-white px-6 py-2 rounded-lg font-semibold hover:bg-blue-700 transition flex items-center gap-2 disabled:bg-slate-300"
            >
              {loading ? <Loader2 className="animate-spin w-4 h-4" /> : <Database className="w-4 h-4" />}
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center">
              <h3 className="font-bold text-slate-800">Questions to Submit</h3>
                  ...
                  <tr key={i} className="group hover:bg-slate-50 transition-colors border-b border-slate-200">
                    <td className="px-6 py-4 text-sm text-slate-600 leading-relaxed">
                      {editingId === `${i}` ? (
                        ...
                        <p className="whitespace-pre-wrap break-words max-h-32 overflow-y-auto">{q.content}</p>
                      )}
                    </td>
                          ...
                          <button 
                            onClick={() => {
                              setQuestions(questions.map((question, idx) => 
                                idx === i ? { ...question, content: editContent } : question
                              ));
                              setEditingId(null);
                              setEditContent("");
                            }}
            ...
            {questions.length === 0 && (
              <div className="py-20 text-center flex flex-col items-center gap-3">
                ...
                <p className="text-slate-400 text-sm">No questions added yet. Add one above.</p>
              </div>
            )}
          </div>
          )}
